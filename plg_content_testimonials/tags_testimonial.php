...
class plgContentTags_testimonial extends JPlugin
{
    public function onContentPrepare($context, &$article, &$params, $page = 0)
    {
        ...
        if ($matches) {
            ...
            if (JFactory::getUser()
                ->authorise('core.admin', 'com_testimonials')
            ) {
                $app = JFactory::getApplication();
                $app->enqueueMessage(JText::_("PLG_CONTENT_TESTIMONIALS_WARNING_OLD_PLUGIN_MARKUP"),
                    "warning");
            }
        ...
        if ($matches) {
            ...
            foreach ($matches as $match) {
                ...
                if (strpos($matcheslist[1], 'tag:') === 0) {
                    ...
                    $categories = JHtml::_('category.options',
                        'com_testimonials', $config = array(
                            'filter.published' => array(1),
                            'filter.language'  => array('*', $language_tag),
                            'filter.access'    => array(1)
                        ));
    ...
    protected function replace($article, $from, $to = '')
    {
        ...
        $article->introtext = preg_replace("|" . addcslashes($from, '|') . "|",
            addcslashes($to, '\\$'), $article->introtext, 1);
        $article->fulltext  = preg_replace("|" . addcslashes($from, '|') . "|",
            addcslashes($to, '\\$'), $article->fulltext, 1);
        $article->text      = preg_replace("|" . addcslashes($from, '|') . "|",
            addcslashes($to, '\\$'), $article->text, 1);
    ...
    protected function prepareClasses()
    {
        ...
        JLoader::register('TestimonialsModelComment',
            JPATH_SITE . '/components/com_testimonials/models/comment.php');
        JLoader::register('TestimonialsModelTestimonials', JPATH_SITE
