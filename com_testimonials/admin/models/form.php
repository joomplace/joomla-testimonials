...
class TestimonialsModelForm extends TestimonialsModelTopic
{
    public function getCustomFields()
    {
        ...
        if ($error) {
            $posted_data = JFactory::getApplication()
                ->getUserState('com_testimonials.edit.form.data', array());
            foreach ($fields as &$field) {
                if ($field->type == 'url') {
                    $field->value = array('', '');
                    if (!empty($posted_data['customs_link'][$field->id])) {
                        $field->value[0]
                            = $posted_data['customs_link'][$field->id];
                    }
                    if (!empty($posted_data['customs_name'][$field->id])) {
                        $field->value[1]
                            = $posted_data['customs_name'][$field->id];
                    }
                    $field->value = implode('|', $field->value);
                } else {
                    if (!empty($posted_data['customs'][$field->id])) {
                        $field->value = $posted_data['customs'][$field->id];
                    }
                }
            }
            unset($field);
    ...
    public function getTags()
    {
        ...
        if ($id > 0) {
            ...
            foreach ($sel as $tag_id) {
                $return->selected[] = $tag_id->id_tag;
            }
        }
