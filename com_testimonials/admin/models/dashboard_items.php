...
class TestimonialsModelDashboard_items extends JModelList
{
	...
	protected function getListQuery() 
	{
		...
		$query->from('`#__tm_testimonials_dashboard_items`');
    ...
    function delete($cid)
    {
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->delete('#__tm_testimonials_dashboard_items');
    ...
    public function publish($cid, $value = 1){
        ...
        if (!is_array( $cid ) || count( $cid ) < 1) {
            ...
            echo "<script> alert('".JText::_('COM_TESTIMONIALS_SELECT_AN_ITEM_TO')." $action'); window.history.go(-1);</script>\n";
            exit();
        }

        $cids = implode( ',', $cid );

        $query = "UPDATE #__tm_testimonials_dashboard_items"
