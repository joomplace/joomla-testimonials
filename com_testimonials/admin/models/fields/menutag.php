...
class JFormFieldMenuTag extends JFormFieldGroupedList
{
    ...
    protected function getInput()
    {
        ...
        $attr .= ((string)$this->element['disabled'] == 'true')
            ? ' disabled="disabled"' : '';
    ...
    protected function getGroups()
    {
        ...
        $published = $this->element['published'] ? explode(',',
            (string)$this->element['published']) : array();
        $disable   = $this->element['disable'] ? explode(',',
            (string)$this->element['disable']) : array();

        // Get the menu items.
        $items = MenusHelper::getMenuLinks($menuType, 0, 0, $published);

        $groups[][] = JHtml::_('select.option', 0,
            JText::_('COM_TESTIMONIALS_SELECT_ITEMS'), 'value', 'text');
        ...
        if ($menuType) {
            ...
            foreach ($items as $link) {
                $groups[$menuType][] = JHtml::_('select.option', $link->value,
