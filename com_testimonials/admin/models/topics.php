...
class TestimonialsModelTopics extends JModelList
{
        ...
        if ($tags) {
            if ($tagopt == 1) {
                $query->where('(c.`id_tag` IN (' . $tags
                    . ')  OR c.id_tag IS NULL OR c.id_tag =0)');
            } else {
                if ($tagopt == 2) {
                    $query->where('c.`id_tag` IN (' . $tags . ')');
                }
    ...
    protected function getListQuery()
    {
        ...
        $tagquery
            = '(SELECT GROUP_CONCAT(`tag_name`) FROM `#__tm_testimonials_tags` WHERE id IN (SELECT id_tag
        ...
        $query->from('`#__tm_testimonials` AS `t`');
        $query->join('LEFT',
            '`#__categories` AS `c` ON `c`.`id` = `t`.`catid` ');

        // Filter by search in title
        $search = $this->getUserStateFromRequest('topics.search',
            'filter_search');
