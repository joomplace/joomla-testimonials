 ...
 * Assign Table class
 */
class TestimonialsTableAssign extends JTable
{
        /**
         * Constructor
         *
        ...
        {
                parent::__construct('#__tm_testimonials_tag_assign', 'tag_id', $db);
        }
        
    	public function loadIds($keys = null, $type='menu')
		{
			if (empty($keys)) {
     ...
     public function storeWithType($updateNulls = false, $type='menu')
	{
	...
	public function deleteWithType($pk = null, $menuid=0, $type='menu')
	{
		// Initialise variables.
		$k = $this->_tbl_key;
		...
		$query->where('aid = '.$this->_db->quote($menuid));
		$query->where('`type` = '.$this->_db->quote($type));
		$this->_db->setQuery($query);
